 ...
 */
export function suggestIntroFraming(
  entity_a: string,
  entity_b: string,
  edges: GraphEdge[],
  profiles?: Record<string, PsychProfile>,
): FramingResult {
  // Calculate affinity from edges
  const affinityEdge = edges.find(
    e => e.metadata?.entity_a === entity_a && e.metadata?.entity_b === entity_b
      || e.metadata?.entity_a === entity_b && e.metadata?.entity_b === entity_a
  );

  const affinity = affinityEdge?.metadata?.affinity_score ?? 50;
  const frictionRisks: string[] = affinityEdge?.metadata?.friction_risks
    ? [String(affinityEdge.metadata.friction_risks)]
    : [];

  // Determine tier
  let tier: FramingResult["tier"];
  if (affinity >= 80) tier = "natural-fit";
  else if (affinity >= 50) tier = "workable";
  else if (affinity >= 20) tier = "caution";
  else tier = "avoid";
