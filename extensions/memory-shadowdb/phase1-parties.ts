...
export interface DbClient {
   ...
   */
  query(sql: string, params?: unknown[]): Promise<{ rows: ContactRow[] }>;
}
...
/**
 * A normalized contact entry for matching.
 */
interface IndexedContact {
  readonly id: number;
  /** Extracted name (everything before " — ") normalized to lowercase */
  readonly normalizedName: string;
  /** Individual tokens of the normalized name */
 ...
 */
function buildContactIndex(rows: ContactRow[]): IndexedContact[] {
  return rows.map((row) => {
 ...
 */
function matchParty(party: string, index: IndexedContact[]): ResolvedParty {
  if (!party.trim()) return { name: party, memoryId: null, matchScore: SCORE_NO_MATCH };
...
function computeMatchScore(
  ...
  if (queryNorm === cn || queryStrip === cn) return SCORE_EXACT;

  // 2. Match after stripping suffixes from contact name too
  const cnStrip = stripSuffix(cn);
  if (queryStrip === cnStrip) return SCORE_SUFFIX_STRIP;
