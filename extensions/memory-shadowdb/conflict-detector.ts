 ...
 */
export function detectConflicts(edges: GraphEdge[]): ConflictResult[] {
  ...
  for (const [key, pairEdges] of byPair) {
    ...
    const types = new Map<string, GraphEdge>();
    for (const edge of pairEdges) {
      const t = edge.metadata?.relationship_type;
      if (t && !types.has(t)) types.set(t, edge);
    }
      ...
      for (const typeB of conflicting) {
        ...
        if (edgeB && edgeA.id !== edgeB.id) {
          ...
          const alreadyReported = conflicts.some(
            c => c.entity_a === first && c.entity_b === second &&
