 ...
 * @returns Score 0-100
 */
export function computeAuthoritySensitivity(profile: PsychProfile | null | undefined): number {
  if (!profile) return 50; // Medium for missing profile
  ...
  if (disc.includes("C")) score += 10; // Conscientiousness = rule-following
  if (disc.includes("S")) score += 5;  // Steadiness = hierarchy-respecting
  if (disc.includes("D")) score -= 5;  // Dominance = challenges authority
