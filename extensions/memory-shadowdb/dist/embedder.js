...
export class EmbeddingClient {
     ...
     */
    async embed(text, purpose = "query") {
        let embedding;
        ...
        return validateEmbeddingDimensions(embedding, this.dimensions, `${this.provider}:${this.model}`);
    }
    /**
    ...
    async embedOllama(text, taskPrefix) {
     ...
     */
    async embedOpenAICompatible(text) {
        ...
        if (!response.ok) {
            ...
            throw new Error(`${this.provider} embedding failed: ${response.status} ${response.statusText}${errText ? ` — ${errText.slice(0, 300)}` : ""}`);
        }
        const data = (await response.json());
    ...
    async embedVoyage(text) {
        ...
        if (!response.ok) {
            const errText = await response.text().catch(() => "");
            throw new Error(`Voyage embedding failed: ${response.status} ${response.statusText}${errText ? ` — ${errText.slice(0, 300)}` : ""}`);
        }
        const data = (await response.json());
    ...
    async embedGemini(text) {
        ...
        if (!response.ok) {
            const errText = await response.text().catch(() => "");
            throw new Error(`Gemini embedding failed: ${response.status} ${response.statusText}${errText ? ` — ${errText.slice(0, 300)}` : ""}`);
        }
        const data = (await response.json());
