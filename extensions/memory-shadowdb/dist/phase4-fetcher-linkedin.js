 ...
 */
export function parseThreadMessages(html, threadId) {
    try {
        const root = parse(html);
 ...
 */
export function threadToExtractedContent(thread) {
    ...
    const date = thread.messages.reduce((latest, m) => (m.sentAt > latest ? m.sentAt : latest), thread.messages[0].sentAt);
...
//# sourceMappingURL=phase4-fetcher-linkedin.js.map