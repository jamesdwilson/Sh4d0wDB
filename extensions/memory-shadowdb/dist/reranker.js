 ...
 */
export async function rerankCandidates(query, candidates, config, logger) {
    ...
    if (candidates.length < config.minCandidates) {
        logger.info(`memory-shadowdb/reranker: skipped — ${candidates.length} candidates < minCandidates=${config.minCandidates}`);
        return sortByRrf(candidates);
    }
    const start = Date.now();
    try {
        const documents = candidates.map((c) => c.content.length > MAX_CONTENT_CHARS
        ...
        const response = await fetchWithTimeout(`${config.baseUrl}/v1/rerank`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body,
        }, config.timeoutMs);
        if (!response.ok) {
            logger.warn(`memory-shadowdb/reranker: HTTP ${response.status} from /v1/rerank — falling back to RRF order`);
            return sortByRrf(candidates);
        }
        const data = (await response.json());
        if (!data?.results || !Array.isArray(data.results)) {
            logger.warn(`memory-shadowdb/reranker: unexpected response shape — falling back to RRF order`);
            return sortByRrf(candidates);
        }
        ...
        logger.info(`memory-shadowdb/reranker: reranked ${candidates.length} candidates in ${elapsed}ms`);
        return scored;
    }
    catch (err) {
        const elapsed = Date.now() - start;
        const msg = err instanceof Error ? err.message : String(err);
        logger.warn(`memory-shadowdb/reranker: error after ${elapsed}ms (${msg}) — falling back to RRF order`);
        return sortByRrf(candidates);
    }
}
/**
 ...
 */
export async function checkRerankerHealth(config) {
    try {
        const response = await fetchWithTimeout(`${config.baseUrl}/health`, { method: "GET" }, HEALTH_TIMEOUT_MS);
        return response.ok;
    }
    catch {
        return false;
 ...
 */
export function parseRerankerConfig(pluginConfig) {
    ...
    try {
        ...
        throw new TypeError(`reranker: invalid baseUrl "${rawUrl}" — must be a valid HTTP(S) URL`);
    }
    // timeoutMs
    const timeoutMs = raw.timeoutMs ?? 3000;
    if (typeof timeoutMs !== "number" || timeoutMs < 0) {
        throw new TypeError(`reranker: invalid timeoutMs ${timeoutMs} — must be a non-negative number`);
    }
    // minCandidates
    const minCandidates = raw.minCandidates ?? 3;
    if (typeof minCandidates !== "number" || minCandidates < 1) {
        throw new TypeError(`reranker: invalid minCandidates ${minCandidates} — must be >= 1`);
    }
