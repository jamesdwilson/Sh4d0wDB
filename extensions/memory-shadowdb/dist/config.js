 ...
 */
export function loadShadowDbConfig(configPath) {
    const tryPaths = [
        configPath,
 ...
 */
export function resolveConnectionString(pluginCfg) {
    ...
    throw new Error("memory-shadowdb: no database configuration found. " +
 ...
 */
export function resolveEmbeddingConfig(pluginCfg) {
    ...
    const apiKeyByProvider = provider === "openai" || provider === "openai-compatible"
    ...
    const baseUrlDefaultByProvider = {
        ollama: ollamaUrl,
...
export function resolvePrimerConfig(pluginCfg) {
    ...
    const maxChars = typeof primer.maxChars === "number" && Number.isFinite(primer.maxChars) && primer.maxChars > 0
    ...
    const cacheTtlMs = typeof primer.cacheTtlMs === "number" && Number.isFinite(primer.cacheTtlMs) && primer.cacheTtlMs >= 0
 ...
 */
export function resolveMaxCharsForModel(primerCfg, model) {
 ...
 */
export function validateEmbeddingDimensions(embedding, expectedDimensions, providerModelLabel) {
    ...
    if (expectedDimensions > 0 && embedding.length !== expectedDimensions) {
        throw new Error(`Embedding dimension mismatch for ${providerModelLabel}: expected ${expectedDimensions}, got ${embedding.length}. ` +
