 ...
 */
export function buildImsgMessageId(chatId, messageId) {
 ...
 */
export function parseImsgChats(ndjson, watermark) {
    const chats = [];
    for (const line of ndjson.split("\n")) {
        ...
        if (!trimmed)
            continue;
        try {
            const chat = JSON.parse(trimmed);
            if (!chat?.id || !chat?.identifier)
                continue;
            if (watermark) {
                const lastMsg = new Date(chat.last_message_at);
                if (isNaN(lastMsg.getTime()) || lastMsg <= watermark)
                    continue;
            }
            chats.push(chat);
        }
 ...
 */
export function parseImsgHistory(ndjson, watermark) {
    const messages = [];
    for (const line of ndjson.split("\n")) {
        const trimmed = line.trim();
        if (!trimmed)
        ...
        try {
            ...
            if (!msg.text || msg.text.trim().length < MIN_TEXT_LENGTH)
                continue;
            // Skip reactions and metadata messages
            if (REACTION_PATTERNS.some(p => p.test(msg.text)))
                continue;
            // Apply watermark filter
            if (watermark) {
                const msgDate = new Date(msg.created_at);
                if (isNaN(msgDate.getTime()) || msgDate <= watermark)
                    continue;
            }
            messages.push(msg);
        }
 ...
 */
export function imsgMessageToExtractedContent(msg, chat) {
...
export class IMessageFetcher {
    ...
    async getNewMessageIds(watermark) {
        ...
        if (this.config.maxChats > 0)
            chats = chats.slice(0, this.config.maxChats);
        const ids = [];
        ...
        for (const chat of chats) {
            try {
                const maxFlag = this.config.maxPerChat > 0
                    ...
                    : "--limit 999";
                const historyOutput = execSync(`imsg history --chat-id ${chat.id} ${maxFlag} --json`, { timeout: 15_000, encoding: "utf8" });
                const messages = parseImsgHistory(historyOutput, watermark);
                const chatCtx = { identifier: chat.identifier, name: chat.name };
...
//# sourceMappingURL=phase1-fetcher-imsg.js.map