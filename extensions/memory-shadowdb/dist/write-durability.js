 ...
 */
export function createOperationsLog() {
...
export class OperationsLog {
    ...
    getOrphans() {
        try {
            ...
            const pendingIds = new Set(pendingLines.map(line => JSON.parse(line).operationId));
            const completedIds = new Set(completedLines.map(line => JSON.parse(line).operationId));
 ...
 */
export function createPendingEntry(category, contentHash) {
    return {
 ...
 */
export function createCompletedEntry(id, category, operationId) {
    return {
        timestamp: new Date().toISOString(),
        operationId,
        operation: 'write',
