 ...
 */
export async function onNewContactSignal(contactId, content, dossier, llm) {
    try {
 ...
 */
export async function extractBehavioralSignals(text, context, llm) {
    try {
 ...
 */
export function computePsychographicDelta(existing, signals) {
    ...
    if (changedDimensions.length === 0 || maxConfidence < DELTA_THRESHOLD)
        return null;
 ...
 */
function extractProfileFromDossier(dossier) {
 ...
 */
function deriveWarmthFromDisc(disc) {
    if (!disc)
        return undefined;
    if (disc === "I" || disc === "S")
        return "high";
 ...
 */
function buildShiftSummary(dimensions, before, after) {
 ...
 */
function buildDeltaSummary(contactTitle, signals, delta) {
    const name = contactTitle.split(/\s+[—–-]\s+/)[0].trim();
    if (delta)
        return `${name}: ${delta.summary}`;
    const highlights = [];
    if (signals.commitmentLanguage.length > 0)
        highlights.push("new commitment language");
    if (signals.toneShifts.length > 0)
        highlights.push("tone shift");
    if (signals.unexpectedTopics.length > 0)
        highlights.push("unexpected topic");
 ...
 */
function computeSignalConfidence(signals) {
    let score = 0.1;
    if (signals.commitmentLanguage.length > 0)
        score += 0.15 * Math.min(signals.commitmentLanguage.length, 3);
    if (signals.toneShifts.length > 0)
        score += 0.10;
    if (signals.unexpectedTopics.length > 0)
        score += 0.10;
    if (signals.deferenceSignals.length > 0)
        score += 0.05;
    return Math.min(score, 0.90);
}
//# sourceMappingURL=phase3-contact-signal.js.map