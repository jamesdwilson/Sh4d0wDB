...
const TIER_MIN_CONTEXT: Record<LlmTier, number> = {
...
export class LlmRouter implements TieredLlmClient {
   ...
   */
  async run(task: LlmTask): Promise<string> {
   ...
   */
  private selectModels(tier: LlmTier): ModelConfig[] {
   ...
   */
  private async callModel(model: ModelConfig, task: LlmTask): Promise<string> {
   ...
   */
  private buildRequestBody(model: ModelConfig, task: LlmTask): Record<string, unknown> {
    const maxTokens = Math.min(
      task.maxTokens ?? model.outputLimit,
      model.outputLimit,
    );
   ...
   */
  private buildHeaders(model: ModelConfig): Record<string, string> {
    return {
  ...
  private parseResponse(text: string): string {
    ...
    const text2 = (content as Record<string, unknown>)?.content;
