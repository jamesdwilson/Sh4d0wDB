...
// ============================================================================
// Re-export types from ARCHITECTURE.md contracts
// ============================================================================

export type EntityNodeType = "person" | "company" | "group" | "fund" | "school" | "event";

export type EdgeType =
  | "knows" | "referred" | "co_invested" | "mentioned" | "tension" | "reports_to"
  | "works_at" | "worked_at" | "invested_in" | "advises" | "founded"
  | "member_of" | "attended"
  | "acquired" | "competes_with" | "partners_with" | "raised_from" | "portfolio_of";
 ...
 */
export function parseContactProfile(html: string, username: string): LinkedInProfile | null {
  try {
...
export function profileToExtractedContent(profile: LinkedInProfile): ExtractedContent | null {
  ...
  for (const exp of profile.experience) {
    if (exp.company && !parties.includes(exp.company)) parties.push(exp.company);
  }
...
export function extractEdgeSignals(profile: LinkedInProfile, selfName: string): EdgeSignal[] {
  ...
  try {
    ...
    const fromCandidate: EntityCandidate = {
      type: "person",
    ...
    for (const exp of profile.experience) {
      ...
      const edgeType: EdgeType = isCurrent ? "works_at" : "worked_at";
    ...
    for (const rec of profile.recommendations) {
      if (!rec.authorName || rec.authorName === selfName) continue;
      if (rec.direction !== "received") continue;
    ...
    for (const conn of profile.sharedConnections) {
      if (!conn || conn === selfName) continue;
      signals.push({
        fromCandidate,
        toCandidate: {
          type: "person",
...
function parseAbout(root: HTMLElement): string | undefined {
  ...
  for (const span of spans) {
    ...
    if (text.length > 20 && text !== "About") return text;
  }
  return undefined;
}

function parseExperienceSection(root: HTMLElement, sectionId: string): LinkedInExperience[] {
  ...
  try {
    ...
    const section = (anchor.closest("section") ?? anchor.parentNode) as HTMLElement | null;
    ...
    for (const li of items) {
      try {
        ...
        for (const span of allNormal) {
          ...
          const parentClass = (parent as HTMLElement)?.classNames ?? "";
...
function parseEducationSection(root: HTMLElement, sectionId: string): LinkedInEducation[] {
  const results: LinkedInEducation[] = [];
  try {
    const anchor = root.querySelector(sectionId);
    if (!anchor) return results;
    const section = (anchor.closest("section") ?? anchor.parentNode) as HTMLElement | null;
...
function parseRecommendations(root: HTMLElement): LinkedInRecommendation[] {
  const results: LinkedInRecommendation[] = [];
  try {
