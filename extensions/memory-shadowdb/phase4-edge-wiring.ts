 ...
 */
export async function processEdgeSignals(
  signals: EdgeSignal[],
  resolver: EdgeResolver,
): Promise<EdgeWiringSummary> {
  const summary: EdgeWiringSummary = { resolved: 0, edges: 0, errors: 0 };
  ...
  for (const signal of signals) {
    try {
      ...
      if (fromEntity) summary.resolved++;
      if (toEntity) summary.resolved++;
